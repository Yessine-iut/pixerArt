...
export const Signin = () => {
	...
	const handleSubmit = async (e) => {
		...
		if (user.username === '')
			...
			try {
				...
				await request.post('http://localhost:8080/api/users/', user)
					.then((resp) => resp.data);
				setSuccess(true);
			} catch (err) {
				setError(err);
	...
	return (
		<React.StrictMode>
			<Row tag="section" className = {`Signsection ${storageMode ? 'dark' : 'light'}`}>
				<NavPixer user={null} />
					<h1>Sign in</h1>
					<Form onSubmit={handleSubmit} className="Signin  align-items-start">
						...
						<FormGroup className="d-flex flex-column align-items-start">
							...
							<Input type="password" name="mdp" onChange={handleChange} placeholder="password" />
						</FormGroup>
						<FormGroup className="d-flex flex-column align-items-start">
